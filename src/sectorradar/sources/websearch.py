...
class AnthropicSearchProvider:
    ...
    Like Vertex grounding, this is a model that searches rather than an index
    you query, so it needs no separate search API key — the same credential
    that runs extraction runs discovery. Results come back as citations on the
    answer, which is what we want: a URL somebody's index actually returned,
    not a URL a model composed.
    """
    ...
    def search(self, query: str, limit: int) -> list[SearchHit]:
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        f"List up to {limit} distinct companies with their own websites. "
                        "For each, give the company name and its homepage URL. "
                        "Skip directories, listicles and news articles — only companies."
                    ),
                }
            ],
        )
        ...
        for block in response.content:
            # Citations carry the URLs the search tool genuinely returned. Any
            # URL only present in the prose is a URL the model may have written
            # from memory, and this pipeline does not follow those.
            for citation in getattr(block, "citations", None) or []:
                url = getattr(citation, "url", None)
                if not url or url in seen:
                    continue
                seen.add(url)
                hits.append(SearchHit(title=getattr(citation, "title", "") or url, url=url))
                if len(hits) >= limit:
                    return hits
        return hits
...
def get_provider(settings: Settings) -> SearchProvider:
    ...
    if choice == "vertex_grounding":
        ...
        return VertexGroundingProvider(
            settings.gcp_project, settings.gcp_location, settings.model_for_search()
        )
    ...
    if choice == "anthropic":
        if not settings.anthropic_api_key:
            msg = (
                "SECTORRADAR_SEARCH_PROVIDER is 'anthropic' but ANTHROPIC_API_KEY is unset in .env"
            )
            raise ConfigError(msg)
        return AnthropicSearchProvider(settings.anthropic_api_key, settings.model_for_search())

    msg = (
        f"unknown SECTORRADAR_SEARCH_PROVIDER '{settings.search_provider}'. "
        "Supported: vertex_grounding, anthropic, exa, brave, tavily."
    )
    raise ConfigError(msg)
