...
class Ctx:
    ...
    settings: Settings
    limit: int | None = None
    #: An open database connection, for the rare source that builds on what
    #: earlier runs found rather than going out cold. Most sources ignore it.
...
def _install() -> None:
    ...
    """
    from sectorradar.sources import (
        directories,
        jobads,
        lindas,
        registry_lookup,
    ...
    register("registry_lookup", registry_lookup.run)
