...
if [ -f src/sectorradar/stats.py ]; then
  ...
  recall=$(uv run sectorradar stats --segment "${SEGMENT:-pilates-zurich}" --recall-only 2>/dev/null | tr -d ' %')
...
SECOND_SEGMENT="${SECOND_SEGMENT:-ai-assurance-ch}"
if [ -f "segments/$SECOND_SEGMENT.yaml" ]; then
  expect_ge "$(q "select count(*) from membership where segment_slug='$SECOND_SEGMENT';")" \
  ...
  skip "phase 5 not reached (no segments/$SECOND_SEGMENT.yaml)"
fi
