...
`segments/genai-training-ch.yaml` is the proof of that promise, not just an
example. It was added with zero changes to any module under `src/`, and it is
a genuinely different segment rather than `agentic-ai-ch.yaml` with the nouns
swapped: its `inclusion` rule admits training providers and solo consultants
that `agentic-ai-ch` deliberately excludes, and its `facets` describe how
...
A good example, from `agentic-ai-ch.yaml`:
...
`agentic-ai-ch` tier 1 is "Agentic AI or GenAI services are the primary
offering"; `genai-training-ch` tier 1 is "GenAI training and enablement is
...
`agentic-ai-ch` enriches `[1, 2]` and leaves tier 3/4 as candidates only.
`genai-training-ch` enriches `[1, 2, 3]` because in that segment tier 3
...
Facets are fixed; values are open. The classifier may emit a new value
inside a facet you have already declared — for example your `tech` facet
lists `[langchain, langgraph, openai, ...]` and the model tags a company
`bedrock` because that's what its site actually says — but it may never
invent a new facet name that wasn't in the YAML. The list of values under
each facet key is a starting vocabulary to anchor the model's output, not an
enum it is restricted to. If you want a facet closed instead of open, that is
a change to `classify.py`'s prompt construction, not something expressible in
the YAML today.
...
- **`seeds`** — a hand-curated list under `urls:`, either bare strings or
  ...
  "we also considered X" list from lost pitches belong here and are the
...
- **`websearch`** — runs each string in `queries:` through a configured
  ...
  segment's market actually operates in — `agentic-ai-ch` runs its queries in
...
`agentic-ai-ch.yaml`. Be honest that directory sites (Clutch, GoodFirms,
...
find the market". `agentic-ai-ch.yaml`'s own comments are explicit about
...
channel discovery doesn't use: an owner's referral graph, a competitor list
from lost pitches, or a manually curated list from someone who knows the
market. `genai-training-ch.yaml` keeps its gold set deliberately small for
