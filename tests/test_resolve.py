...
@pytest.mark.parametrize(
    ...
    [
        "Vogler Consulting",
        "Vogler Consulting GmbH",
        "Vogler Consulting AG",
        "Vogler Consulting Sàrl",
        "Vogler Consulting SA",
        "Vogler Consulting Sagl",
        "Vogler Consulting Ltd.",
        "Vogler Consulting LLC",
        "  Vogler   Consulting  gmbh ",
        "VOGLER CONSULTING AG",
    ],
)
def test_legal_suffixes_normalise_to_the_same_name(raw: str) -> None:
    ...
    assert resolve.normalise_name(raw) == "vogler consulting"
...
def test_a_near_identical_name_on_another_domain_is_flagged_not_merged(
    ...
    """
    _candidate(conn, "Vogler Consulting GmbH", "https://vogler-consulting.ch")
    _candidate(conn, "Vogler Consulting AG", "https://voglerconsulting.ch")
    conn.commit()
    ...
    assert "vogler-consulting.ch" in flagged[0]["review_note"]
